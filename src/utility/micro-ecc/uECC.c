...
int uECC_sign_with_k(const uint8_t *private_key,
    ...
    uECC_word_t s2[uECC_MAX_WORDS]; // n-s
    ...
    s2[num_n_words - 1] = 0;
    uECC_vli_set(s2, s, num_words);
    uECC_vli_sub(s2, curve->n, s2, num_n_words);
    if(uECC_vli_cmp(s2,s, num_n_words)==-1){
        uECC_vli_set(s,s2,num_n_words);
    }
#if uECC_VLI_NATIVE_LITTLE_ENDIAN
